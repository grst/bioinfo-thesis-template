...
\begin{algorithm}[h!]
 ...
 $v_c = rows/2$;  \\
 Define $\bv F_{ray}$; \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 
 // points to the center of desired perspective image \\
 $\bv F_{ray} = \bv F_{ray}.normalize()$ * focal length; \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \\
 %// set $\norm {\bv F_{ray}} $ to focal length \\
 Define $\bv u$ and $\bv v$; \ \ \ \ \ \ \ \  
 // orthogonal to $\bv F_{ray}$, both vectors define image plane \\
 $\bv u$.normalize(); \\
 $\bv v$.normalize(); \\
 \For{v = 0 to rows}
 {
...
In practice the rays will always be the same once parameters are set.  Therefore the rays are pre-calculated in the form of a perspective pixel coordinate to omni pixel coordinate lookup table.
...
\subsection{Introducing loop closure constraints to the SLAM graph}
